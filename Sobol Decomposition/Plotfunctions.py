...
def rescale_size(s, max_val):
    if max_val <= 0:
        # If max absolute value is 0, all values are 0. Return a small constant size.
        return np.full_like(s, 2)
        
    s_normalized = s / max_val
    s_scaled = s_normalized ** 0.3
    # The final multiplier is adjusted for better visual effect in the new context.
    return s_scaled * 65
...
def boxplots(Var, colory, Year, XRdata, XRmeta, modelorcat, title, remc8):
    ...
    for m in range(len(unimodels)):
        ...
        traces_m.append(go.Box(y=y, name=unimodels[m2][:3], fillcolor=col, line_color='black', marker=dict(color=col), boxpoints='all', jitter=0.5, pointpos=-1.8, whiskerwidth=0.2, marker_size=1.5, line_width=1, showlegend=False))
